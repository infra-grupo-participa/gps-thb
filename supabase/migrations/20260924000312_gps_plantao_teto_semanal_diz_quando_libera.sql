-- A mensagem do teto semanal dizia o que o aluno NAO pode, nunca quando volta
-- a poder. Quem participa na segunda le "Voce ja tem plantao nesta semana" o
-- resto da semana inteira, sem saber se sao 2 dias ou 2 meses de espera --
-- e escreve para o suporte. Um aluno com 6 presencas em 6 fez exatamente isso.
--
-- Passa a nomear a data: a semana do conflito + 7 dias, sempre a segunda
-- seguinte. Calculado do conflito que a funcao ja leu, nao de now(): se o
-- aluno estiver olhando um slot de outra semana, a data continua certa.
--
-- Nada de regra muda. So o texto. O teto, o interruptor `teto_semanal_ativo`
-- e o indice uq_plantao_inscricoes_aluno_semana continuam identicos.
...
declare
  ...
  v_msg_semana text := 'Você já tem plantão nesta semana. É um plantão por semana — as datas da semana seguinte abrem na próxima segunda-feira.';
begin
    ...
    if found then
      ...
      return query select false,
        ('Você já tem plantão nesta semana (' ||
         ...
         '). É um plantão por semana — a partir de ' ||
         to_char(gps.plantao_semana(v_conflito.data) + 7, 'DD/MM') ||
         ' você já pode escolher a próxima data.')::text,
        null::uuid, null::text, null::text, null::date, null::time, null::text;
      return;
    end if;
  end if;
