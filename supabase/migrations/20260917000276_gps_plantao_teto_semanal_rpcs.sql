-- Aplica o teto semanal nas DUAS rotas de inscricao. A rota logada precisa
-- da regra identica a publica: quando so a publica tinha a trava de prazo,
-- a aba logada aceitava quem a publica recusava.
...
begin
  ...
  -- a pessoa remarcava minutos depois. O `for update` no slot acima nao
  -- serializa duas inscricoes em slots DIFERENTES da mesma semana, entao a
  -- trava real e o indice unico criado na migration seguinte; esta
  -- checagem existe para devolver mensagem legivel no caso normal.
