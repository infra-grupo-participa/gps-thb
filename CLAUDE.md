...
Aba **`/pasta`** (e admin `.../pasta`): botão "Abrir no Drive", **pré-visualização embutida**
(`embeddedfolderview`) e o **mapa da estrutura padrão** (`src/lib/pasta.ts` → seções 0–5 tied às
etapas: Contrato, Documentos[Pessoais/Imóveis/Empresas], Vídeos, Croqui, Minutas[Cofre/Veículo/
Destino/Acordo], Entrega). Admin define/edita o link (`salvarPastaDriveUrl`). Item "Pasta" no nav.
